...
class ScreenCapture:
    def __init__(self):
        ...
        self._error_count = 0        # подавляем повторные ошибки
        self._last_error_log = 0.0   # время последнего лога ошибки
    ...
    def capture(self) -> Optional[Screenshot]:
        """Делает скриншот. Пробует mss, затем PIL.ImageGrab."""
        if not self._available:
            return None
        shot = self._capture_mss() or self._capture_pil()
        if shot:
            self._error_count = 0
            with self._lock:
                self._latest = shot
    ...
    def _capture_mss(self) -> Optional[Screenshot]:
        try:
            import mss, cv2
            with mss.mss() as sct:
                ...
                return Screenshot(image=arr, timestamp=time.time(),
                                  width=img.width, height=img.height)
        except Exception:
            return None

    def _capture_pil(self) -> Optional[Screenshot]:
        """Fallback через PIL.ImageGrab — работает без admin прав."""
        try:
            ...
            self._error_count += 1
            # Логируем не чаще чем раз в 60 сек, чтобы не спамить
            now = time.time()
            if now - self._last_error_log > 60:
                logger.warning(f"Screenshot error (×{self._error_count}): {e}")
