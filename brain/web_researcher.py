...
class WebResearcher:
    ...
    EPIC_COMMUNITY_API = "https://dev.epicgames.com/community/api/search"
    SEARCH_ENGINES = [
        "https://docs.unrealengine.com/search/?q={query}",
        "https://dev.epicgames.com/community/search?q={query}&tag=unreal-engine",
    ...
    def research(self, task: str, context: str = "") -> Optional[ResearchResult]:
        ...
        # 3. Ищем в интернете (приоритет: память → Epic docs → Epic community → web → LLM)
        logger.info(f"Researching: {task!r}")
        result = self._search_ue5_docs(task)

        if not result:
            result = self._search_epic_community(task)
    ...
    def _search_epic_community(self, task: str) -> Optional[ResearchResult]:
        """Ищет в Epic Developer Community (туториалы, Q&A, форумы UE5)."""
        query = self._task_to_search_query(task)
        search_url = (
            f"https://dev.epicgames.com/community/search"
            f"?q={quote_plus(query)}&tag=unreal-engine"
        )
        try:
            resp = self._session.get(search_url, timeout=8)
            if not resp.ok:
                return None
            # Пробуем найти ссылки на туториалы
            urls = re.findall(
                r'href="(https://dev\.epicgames\.com/community/learning[^"]+)"',
                resp.text
            )
            urls = list(dict.fromkeys(urls))[:2]
            if not urls:
                return None
            page_resp = self._session.get(urls[0], timeout=10)
            if not page_resp.ok:
                return None
            text = self._extract_text_from_html(page_resp.text)
            if len(text) < 100:
                return None
            return self._extract_steps_from_text(task, text[:2500], urls[0], "epic_community")
        except Exception as e:
            logger.debug(f"Epic Community search failed: {e}")
            return None
